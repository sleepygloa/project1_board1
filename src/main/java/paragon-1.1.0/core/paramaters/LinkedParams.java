  ...
  implements Params, Comparator<DataRow>
{
  ...
  public void createLevel(String dataTableName) {
    ...
    for (DataRow targetDataRow : targetDataTable) {
      originalKey = targetDataRow.getVal(_CUSTOM_KEY_).toString();
      String originalParentKey = targetDataRow.getVal(_CUSTOM_PARENT_KEY_).toString();
      if (paramLinkedCheckMap.containsKey(originalParentKey)) {
        ...
        if ((!_CUSTOM_ORDER_.equals("ROW_ORDER")) || (_CUSTOM_ORDER_.equals("ROW_ORDER"))) {
          childOrderValue = targetDataRow.getVal(_CUSTOM_ORDER_).toString();
        }
    ...
    for (DataRow originalDataRow : targetDataTable) {
      originalDataRow.put("LEVEL", ((Map)paramLinkedCheckMap.get(originalDataRow.getVal(_CUSTOM_KEY_).toString())).get("LEVEL"));
      originalDataRow.put(_CUSTOM_ORDER_KEY_, ((Map)paramLinkedCheckMap.get(originalDataRow.getVal(_CUSTOM_KEY_).toString())).get(_CUSTOM_ORDER_));
      
      originalDataRow.put("ISLEAF", ((Map)paramLinkedCheckMap.get(originalDataRow.getVal(_CUSTOM_KEY_).toString())).get("ISLEAF"));
      originalDataRow.put("CHILE_CNT", ((Map)paramLinkedCheckMap.get(originalDataRow.getVal(_CUSTOM_KEY_).toString())).get("CHILE_CNT"));
  ...
  public int compare(DataRow first, DataRow last)
  {
    ...
    try {
      firstOrderInt = first.getInt(_SORT_COLUNM_);
      LastOrderInt = last.getInt(_SORT_COLUNM_);
    } catch (Exception e) {
      integerFlag = false;
    }
